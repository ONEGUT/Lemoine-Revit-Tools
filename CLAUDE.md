...
| `ModelPath savedMp = doc.SaveAsCloudModel(accountId, projectId, folderId, modelName);` | `SaveAsCloudModel` returns **`void`**, not `ModelPath` (CS0029 if assigned) — after the call, read the resulting cloud path back via `doc.GetCloudModelPath()` |
