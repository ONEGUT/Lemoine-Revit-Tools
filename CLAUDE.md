...
- Build every year in one pass with `dotnet build LemoineTools.csproj -t:BuildAllYears` (the `BuildAllYears` target at the bottom of `LemoineTools.csproj`). It builds each `ReleaseYYYY` Configuration independently and drops each year's output straight into that year's `DeployDir` — a failure in one year (e.g. an unpopulated `libsYYYY/`) is logged as a warning and does not stop the remaining years from building. Check the log for "Build succeeded"/"Build FAILED" under each year's banner to see which ones actually produced output.
