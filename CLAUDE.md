...
- **`Directory.Build.props` sets `BaseIntermediateOutputPath` to `obj\$(Configuration)\`.** Without it, all 8+ Configurations share one `obj\project.assets.json`, and since each year resolves a different `TargetFramework` (net48 vs net8.0-windows), restoring one year clobbers the file another year just restored into — this was the root cause of a `NETSDK1005` ("doesn't have a target for 'net8.0-windows'") failure when `BuildAllYears` built the years back-to-back. Splitting the intermediate path per Configuration keeps each year's restore/build artifacts isolated. `BuildAllYears`'s nested `MSBuild` call also targets `Restore;Build` (not just `Build`), since a raw `<MSBuild>` task invocation doesn't get the CLI's implicit restore-before-build that a plain `dotnet build`/`msbuild` call gets.
