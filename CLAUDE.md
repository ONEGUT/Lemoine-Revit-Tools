...
- **Interleaving `IndependentTag.Create` with geometry reads forces a full model regeneration before every read** — ~2 s per tag, 20 minutes for 580 — and there is no batch tag API (`Creation.Document` has no generic `NewTag`; `PostableCommand.TagAllNotTagged` is modal, async and active-view-only). Structure bulk tagging as read-everything → plan off-model → write-everything so the run costs **one** regeneration. That includes **view-scoped collectors**: `new FilteredElementCollector(doc, viewId)` run per view *inside* the write loop is a read after a write, so an N-view run pays N−1 extra regenerations. Hoist every view's collection into one read phase ahead of the first write.
- **Writing a view-specific element into a CLOSED view regenerates that view for every single element written.** This is a *second*, independent regeneration trap from the read-interleaving one above, and it survived the read→plan→write fix: with the target view open a tag run finishes in ~10 s, with it closed Revit recomputes the view per tag. Revit maintains a computed element/geometry set only for **open** views; a closed one has no such state, so each `Create` computes it and then re-dirties it for the next. Fix: open every target view before the run (`PickerViewGuard.OpenViews`), then close what you opened. Constraints — `UIDocument.ActiveView` is the **only** way to open a view (`RequestViewChange` is deferred until after the API context returns, and there is no hidden/background open, so Revit visibly flips through them); **the active view cannot be changed while a transaction is open**, so all views must be opened *before* `tx.Start()`, never per view inside the loop. *(Cause inferred from the API plus a measured run; the ~10 s figure is the user's, on a Windows/Revit build.)*
...
- **A ring is judged by what is INSIDE it, not by its own shape.** A corridor looping a block of rooms is a corridor **all the way around** (rule 3, one tag per corner-to-corner stretch); a ceiling with an **atrium or shaft** punched through it gets **one tag at the top centre** of the band, explicitly *not* one per side. The discriminator is the fraction of the enclosed hole covered by **other ceiling regions already read for that view** (`TagPlanConfig.RingFillFraction`, 0.5): a corridor rings *rooms*, and rooms have ceilings; an atrium rings a *void*, which has none at that level. Measured — corridor ring 100% (71% when the inner rooms are only partly ceilinged), atrium 0%, atrium with a small canopy in it 5%. Costs no extra model reads, because every ceiling in the view is already in the layer list.
  - **Elongation cannot do this job** — a hole thins any shape, so an atrium ring scores 6.0 and a shaft ring 5.4, both above the 4.0 corridor threshold. Both were classified "corridor" all along; the old unconditional one-tag rule merely *masked* it. Do not try to fix this with a maximum-corridor-width cap either: it would misjudge a genuinely wide circulation spine.
  - **Do not use walls or Revit Rooms for it.** A corridor looping ONE large room has no walls inside its hole at all (they sit on its boundary), while an atrium usually has walls right at its edge — so the two cases that most need separating look identical. Turning wall outlines into room extents means reimplementing Revit's room computation (planar-graph cycle finding over centrelines, room-bounding flags, curtain walls, links). The ceiling-coverage test gets the single-big-room case right at 100%.
  - A **U or L** never reaches this test (no enclosed hole), and a room with a soffit under it is settled earlier by rule 1 on its own footprint (elongation 1.2), so both are unaffected.
- **Corridor** tags are placed **one per corner-to-corner stretch**, subdivided by the spacing setting (a leg of length L gets `ceil(L / spacing)` tags, each centred on its own share). A corner **starts a new count**. This was briefly replaced by a continuous run that carried the accumulated distance across corners — but only because fixture openings were collapsing the measured width and manufacturing legs where there was no real corner; `MinHoleAreaFt2` fixed that cause, and the per-stretch rule was restored on the user's instruction. The continuous rule's real defect: on an L with a 60 ft and a 40 ft arm it gave the short arm **one** tag, off-centre, because the long arm had already consumed the spacing budget. `MinTagSeparationFt` (6 ft) is what keeps the two legs meeting at a corner from placing two tags on top of each other — validated on straight/L/U/Z/ring corridors via the Python port.
...
### Isolating a ceiling plan to its own level

`CeilingLevelFilters` (`Source/Tools/Ceilings/`) is the shared mechanism, used by Ceiling Heatmap's per-level generation and by Make Ceiling Grids. Same shape as the CH/CG trades: an `ExternallyManaged` `CL` / "Ceiling Levels" trade, one `ParameterFilterElement` per level, reused by name and rewritten in place with `SetElementFilter`.

- **A view range cannot express "belongs to this level" — use a Level filter.** A view range is a vertical band; a ceiling belongs to a level by its `LEVEL_PARAM`, and a bulkhead dropped well below its level (or a double-height space's ceiling running past the level above) lands on the wrong side of any band you pick. Applying the filters is: add **every** level's filter to the view, `SetFilterVisibility(own, true)` and `false` for all the others.
- **A parameter that READS a value is not necessarily one a view filter may BIND.** `LEVEL_PARAM` reads a ceiling's level correctly (`CeilingRegionSource`, `ExtendWallsEventHandler` both rely on it), but a parameter can still be refused by `ParameterFilterElement.Create` as not applicable to the filter's categories — the same trap already listed for `ELEM_CATEGORY_PARAM`. A wrong choice fails **every** item identically, which reads as "the whole feature is broken" rather than "one parameter is wrong". Resolve the binding parameter against `ParameterFilterUtilities.GetFilterableParametersInCommon(doc, cats)` at run time (as `AutoFiltersEventHandler.NarrowToFilterable` already does), keep an ordered candidate list, and let a **real write** settle it — the filterable list is necessary but not sufficient. Candidates for "which level does this belong to": `LEVEL_PARAM`, `SCHEDULE_LEVEL_PARAM`, `INSTANCE_SCHEDULE_ONLY_LEVEL_PARAM`, `FAMILY_LEVEL_PARAM`.
- **Never derive a diagnostic about step A from the success list of step B.** `CeilingLevelFilters` counted "did any link level match by name?" from the *written filters* dictionary, so when every write failed it reported "No level in 3 loaded link(s) matched a host level by name or elevation — check that the link's level names match" — sending the user to inspect level names that had matched perfectly. Count each reported fact where it actually happens (the matching pass), and when a step fails wholesale say *that*, with Revit's own message, as its own line.
- **Build a rule for every level in the document, not just the levels being viewed.** A ceiling on a level with no generated view is matched by no filter at all and stays visible.
- **A view with no `GenLevel` must be skipped, not isolated** — no filter would be "its own", so every level would be hidden and the plan would come out blank.
- **`Level` is an ElementId-valued parameter, so a host filter holding the host's level id is not expected to match a ceiling inside a LINK** — whose level is a different element in a different document. That is the dominant case here (ceilings typically live in linked architectural models). The rule is therefore built as a `LogicalOrFilter` over the host level id **plus the id of the equivalent level in every loaded link** (matched by name first, world elevation second), falling back to a host-only rule *with a logged reason* if Revit rejects the foreign ids. **UNVERIFIED — needs a Windows/Revit run.**
- **A view whose filters are governed by a view template refuses both `AddFilter` and `SetFilterVisibility`** — catch, skip, and name the template in the warning (one line per view, not one per level). This bites the heatmap in particular, since its generated RCPs can carry a chosen template.
- **Cross-document level identity is the level NAME, never the ElementId.** Elevation is the fallback, for a link whose level naming differs. This is what Tag Ceilings' check compares, and it reads each ceiling's level from `el.Document` — resolving a linked ceiling's level id against the host returns an unrelated element or nothing.
- **Tag Ceilings warns about strays but still tags them.** The check is a *report* that the isolation did not fully take, not a second filter; silently dropping those tags would hide exactly the thing it exists to surface. Report one line per offending level (bounded by the level count), never one per ceiling.
...
- **`uidoc.ActiveView = view` opens that view in the Revit UI**, and Revit holds every open view's graphics in native RAM for the rest of the session — GC can never reclaim it. Any picker/loop that activates views (e.g. per-view `PickObject`), *and any bulk writer that opens its target views for speed*, must snapshot the open `UIView`s and active view first, then afterwards restore the original active view and close only the views it opened. `PickerViewGuard` (`Source/Framework/`, moved there from the AutoDimension folder once a second tool needed it) is the reference: `Snapshot` → `OpenViews` → `CloseOpenedViews`. Never close a view the user already had open.
