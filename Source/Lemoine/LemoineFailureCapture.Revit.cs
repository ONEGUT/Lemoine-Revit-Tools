...
{
    // =========================================================================
    // LemoineFailureCapture (Revit half) — surfaces Revit's transaction failures
    // and modal dialogs into the active tool run's Output log.
    //
    ...
    //
    // This file is NOT linked into the Navisworks build (it is Revit-only). The
    // shared state (_seen / _gate) and BeginRun() live in LemoineFailureCapture.cs.
    //
    // Behaviour (per user decision):
    //   • Warnings  → logged once, then DeleteWarning'd so no dialog pops.
    //   • Errors    → logged, then left for Revit to show its dialog.
