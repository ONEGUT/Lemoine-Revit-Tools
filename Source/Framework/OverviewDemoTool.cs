...
{
    // ─────────────────────────────────────────────────────────────────────────
    // Catalog-driven, Revit-free IStepFlowTool used by the Tools Overview to give
    ...
    public sealed class OverviewDemoStep
    {
        ...
        public string FilePlaceholder { get; set; } = AppStrings.T("overviewDemo.browsePlaceholder");
    ...
    public sealed class OverviewDemoTool : IStepFlowTool
    {
        ...
        public string Title    => _spec.Title + AppStrings.T("overviewDemo.titleSuffix");
        ...
        private FrameworkElement BuildSearch(OverviewDemoStep s)
        {
            ...
            {
                Items       = s.Items ?? new List<string>(),
                Placeholder = string.IsNullOrEmpty(s.TextPlaceholder) ? AppStrings.T("overviewDemo.searchPlaceholder") : s.TextPlaceholder,
            };
        ...
        private FrameworkElement BuildText(OverviewDemoStep s)
        {
            ...
            {
                Text        = s.TextDefault,
                Placeholder = string.IsNullOrEmpty(s.TextPlaceholder) ? AppStrings.T("overviewDemo.textPlaceholder") : s.TextPlaceholder,
            };
        ...
        public string SummaryFor(string stepId)
        {
            ...
            if (step.Kind == "composite")
            {
                ...
                return parts.Count > 0 ? string.Join(" · ", parts) : AppStrings.T("overviewDemo.ready");
            }
            ...
            switch (step.Kind)
            {
                case "multi":
                    return _multi.TryGetValue(step.Id, out var m) && m.Count > 0
                        ? $"{m.Count} selected" : AppStrings.T("overviewDemo.noneSelected");
                case "file":
                    return _value.TryGetValue(step.Id, out var f) && !string.IsNullOrEmpty(f)
                        ? System.IO.Path.GetFileName(f) : AppStrings.T("overviewDemo.noFile");
                case "single":
                case "search":
                case "text":
                ...
                default:
                    return AppStrings.T("overviewDemo.ready");
            }
        }
        ...
        {
            pushLog(AppStrings.T("overviewDemo.demoBanner"), "info");
            pushLog(string.IsNullOrEmpty(_spec.SampledFrom)
                ? AppStrings.T("overviewDemo.cannedData")
                : AppStrings.T("overviewDemo.sampledFrom", _spec.SampledFrom), "info");
                ...
                case "sel":
                    if (_multi.TryGetValue(id, out var sel) && sel.Count > 0)
                    {
                        ...
                        return string.Join(", ", names)
                            + (extra > 0 ? AppStrings.T("overviewDemo.moreSuffix", extra) : "");
                    }
                    return "—";
                case "first":
