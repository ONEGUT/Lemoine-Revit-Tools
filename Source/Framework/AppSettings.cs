...
namespace LemoineTools.Framework
{
    ...
    public enum UiSize { Small, Medium, Large, ExtraLarge }
    ...
    {
        /// <summary>The single shared instance of <see cref="AppSettings"/>; initialised once and loaded from disk on first access.</summary>
        public static AppSettings Instance { get; } = new AppSettings();

        /// <summary>Currently active colour theme applied to all Lemoine windows.</summary>
        public ThemePalette  ActiveTheme { get; private set; } = ThemePalette.DarkMono;

        /// <summary>Currently active UI size preset that drives the <see cref="Scale"/> multiplier.</summary>
        public UiSize UiSize      { get; private set; } = UiSize.Medium;
        ...
        public event Action<UiSize>? UiSizeChanged;
        ...
        public void SetTheme(ThemePalette theme)
        {
            ...
            ActiveTheme = theme;
            RaiseIsolated(ThemeChanged, theme, "AppSettings.SetTheme");
            SaveToDisk();
        }
        ...
        public void SetUiSize(UiSize size)
        {
            ...
            UiSize = size;
            RaiseIsolated(UiSizeChanged, size, "AppSettings.SetUiSize");
            SaveToDisk();
        }
        ...
        private static void RaiseIsolated<T>(Action<T>? handlers, T arg, string context)
        {
            ...
            {
                try { ((Action<T>)d)(arg); }
                catch (Exception ex) { DiagnosticsLog.Swallowed($"{context}: subscriber threw", ex); }
            }
        }
        ...
        private void LoadFromDisk()
        {
            try
            {
                ...
                using (var r = new StreamReader(path))
                {
                    ...
                    var t = System.Array.Find(ThemePalette.All, x => x.Name == dto.Theme);
                    if (t != null) ActiveTheme = t;
                    if (Enum.TryParse<UiSize>(dto.UiSize, out var s))
                        UiSize = s;
            ...
            {
                DiagnosticsLog.Swallowed("AppSettings: load UISettings.xml (using defaults)", ex);
            }
        }
        ...
        private void SaveToDisk()
        {
            ...
            {
                DiagnosticsLog.Error("AppSettings: save UISettings.xml failed — theme/size will not persist", ex);
            }
        }
    }
}
