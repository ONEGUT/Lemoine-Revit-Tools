...
namespace LemoineTools.Framework
{
    ...
    /// </summary>
    public sealed class ListReorder
    {
        private const string Fmt = "ListReorder.Index";

        private readonly DragGhost _ghost = new DragGhost();
        private readonly StackPanel       _panel;
        private readonly Action<int, int> _onReorder;

        public ListReorder(StackPanel panel, Action<int, int> onReorder)
        {
        ...
        {
            if (row == null) return;
            MotionEffects.WireDragArm(row, e =>
            {
                ...
                {
                    DiagnosticsLog.Swallowed("ListReorder: drag", ex);
                }
                finally
                {
                    row.QueryContinueDrag -= upd;
