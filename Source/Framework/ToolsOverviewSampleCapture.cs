...
namespace LemoineTools.Framework
{
    ...
    //
    // Every pool is wrapped in its own try/catch → DiagnosticsLog.Swallowed so one
    ...
    public static class ToolsOverviewSampleCapture
    {
        ...
        public static OverviewSampleSnapshot? Capture(UIDocument? uidoc)
        {
            ...
            if (doc == null)
            {
                DiagnosticsLog.Info("OverviewSampleCapture", "No active document — demos use canned sample data.");
            ...
            Guard("categories", () =>
            {
                foreach (var (groupKey, cats) in CategoryProbes)
                {
                    ...
                    if (present.Count > 0) snap.CategoryGroups[AppStrings.T(groupKey)] = present;
                }
            });

            DiagnosticsLog.Info("OverviewSampleCapture",
        ...
        {
            try { fill(); }
            catch (Exception ex) { DiagnosticsLog.Swallowed($"OverviewSampleCapture: {what}", ex); }
        }
    }
}
