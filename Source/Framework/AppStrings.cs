...
namespace LemoineTools.Framework
{
    ...
    ///
    /// <para>Revit-free, like <see cref="DiagnosticsLog"/>, so any layer can call it.</para>
    ///
    ...
    public static class AppStrings
    {
        ...
        public static IReadOnlyList<string> AvailableCultures()
        {
            ...
            {
                DiagnosticsLog.Swallowed("AppStrings.AvailableCultures", ex);
            }
            return list;
        }
        ...
        /// (which is also logged via <see cref="DiagnosticsLog"/> so a missing/mistyped key surfaces in
        ...
        public static string T(string key, params object[] args)
        {
            ...
            {
                DiagnosticsLog.Swallowed($"AppStrings.T: bad format template for key '{key}'", ex);
                return text;
            }
        }
        ...
        private static string Resolve(string key)
        {
            ...
            DiagnosticsLog.Warn("AppStrings", $"missing string key: {key}");
            return key;
        }
        ...
        private static IReadOnlyDictionary<string, string> LoadCulture(string culture)
        {
            ...
            {
                DiagnosticsLog.Warn("AppStrings", $"culture folder not found: {dir}");
                return map;
            }
            ...
            foreach (var file in Directory.GetFiles(dir, "*.json"))
            {
                ...
                try
                {
                    ...
                    if (MiniJson.Parse(cleaned) is Dictionary<string, object?> root)
                        ...
                        DiagnosticsLog.Warn("AppStrings", $"file is not a JSON object, skipped: {file}");
                }
                catch (Exception ex)
                {
                    // Skip the bad file rather than aborting the whole load — its keys fall through to English.
                    DiagnosticsLog.Error($"AppStrings: failed to load '{file}'", ex);
                }
            }
            return map;
        }
    ...
    /// upstream by <see cref="AppStrings.StripComments"/> before parsing.
    /// </summary>
