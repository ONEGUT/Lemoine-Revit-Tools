...
namespace LemoineTools.Tools.Testing.PdfRegionWand
{
    ...
    public sealed class PdfToModelTransform
    {
            ...
            Action<string, string>? log)
        {
            ...
            {
                log?.Invoke("Could not determine the placed image size or PDF page size.", "fail");
                return null;
            }
            if (placement.SizeFromRotatedBbox)
                log?.Invoke("Image size was read from the view bounding box of a rotated instance — traced geometry may be offset.", "warn");
            ...
            if (core.ScaleMismatch > 0.01)
                log?.Invoke($"Image placement is stretched non-uniformly ({core.ScaleMismatch:P1} width/height disagreement) — traced geometry will be distorted.", "warn");
