    ...
    public class PdfRegionWandEventHandler : IExternalEventHandler
    {
        ...
        public void Execute(UIApplication app)
        {
            ...
            catch (Exception ex)
            {
                ...
                PushLog?.Invoke(LemoineStrings.T("pdfRegionWand.handler.opFailed", Op, ex.Message), "fail");
            ...
            {
                // Static handler lives for the Revit session — release the per-op
                // payload so no plan/adapter/transform outlives its raise. The
                // palette re-sets all of these before every Raise().
                Plan = null;
                Options = null;
                Adapter = null;
                Transform = null;
        ...
        private void ExecuteSelectImage(UIApplication app)
        {
            ...
            try
            {
                picked = uidoc.Selection.PickObject(ObjectType.Element,
                    new ImageInstanceFilter(), LemoineStrings.T("pdfRegionWand.handler.pickUnderlayPrompt"));
            }
            ...
            {
                PushLog?.Invoke(LemoineStrings.T("pdfRegionWand.handler.notImage"), "fail");
                OnImageSelected?.Invoke(null);
                return;
            }
            ...
            if (bb == null)
            {
                PushLog?.Invoke(LemoineStrings.T("pdfRegionWand.handler.noBoundingBox"), "fail");
                OnImageSelected?.Invoke(null);
                return;
            }
            ...
            if (selection.PdfPath == null)
                PushLog?.Invoke(LemoineStrings.T("pdfRegionWand.handler.pdfUnresolved"), "warn");
            else if (!System.IO.File.Exists(selection.PdfPath))
            {
                PushLog?.Invoke(LemoineStrings.T("pdfRegionWand.handler.pdfMissing", selection.PdfPath), "warn");
        ...
        private void ExecutePickPoint(UIApplication app)
        {
            ...
            try
            {
                var p = uidoc.Selection.PickPoint(LemoineStrings.T("pdfRegionWand.handler.pickSeedPrompt"));
            ...
            catch (Autodesk.Revit.Exceptions.InvalidOperationException ex)
            {
                ...
                PushLog?.Invoke(LemoineStrings.T("pdfRegionWand.handler.noWorkPlane"), "fail");
                OnSeedPicked?.Invoke(null);
            }
        }
        ...
        private void ExecuteStartSplitLines(UIApplication app)
        {
            ...
            if (cmd == null || !uidoc.Application.CanPostCommand(cmd))
            {
                ...
                PushLog?.Invoke(LemoineStrings.T("pdfRegionWand.handler.lineToolUnavailable"), "fail");
        ...
        private void ExecuteFinishSplitLines(UIApplication app)
        {
            ...
            if (doc != null)
            {
                ...
                {
                    using (var tx = new Transaction(doc, LemoineStrings.T("pdfRegionWand.handler.txRemoveSplitLines")))
                    {
                        tx.Start();
                        ...
                        catch (Exception ex)
                        {
                            ...
                            PushLog?.Invoke(LemoineStrings.T("pdfRegionWand.handler.removeLinesFailed"), "warn");
                        }
                    }
                }
            }
        ...
        private void ExecuteCreateRegion(UIApplication app)
        {
            ...
            using (var tx = new Transaction(doc, LemoineStrings.T("pdfRegionWand.handler.txCreate", Adapter.DisplayName, FaceId)))
            {
                tx.Start();
                ConfigureFailures(tx);
                try
                {
                    ...
                    if (ids == null || ids.Count == 0)
                        throw new InvalidOperationException(LemoineStrings.T("pdfRegionWand.handler.noElements"));
                    tx.Commit();
                ...
                catch (Exception ex)
                {
                    ...
                    PushLog?.Invoke(LemoineStrings.T("pdfRegionWand.handler.createFailed", FaceId, Adapter.DisplayName.ToLowerInvariant(), ex.Message), "fail");
                    OnRegionCreated?.Invoke(FaceId, -1);
                }
            }
        }
            ...
            public FailureProcessingResult PreprocessFailures(FailuresAccessor fa)
            {
                                      ...
                                      .Where(m => m.GetSeverity() == FailureSeverity.Warning))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        desc = LemoineStrings.T("pdfRegionWand.handler.unreadableWarning");
                    }
                    if (_seen.Add(desc))
                    {
                        _log?.Invoke(LemoineStrings.T("pdfRegionWand.handler.warningPrefix", desc), "warn");
