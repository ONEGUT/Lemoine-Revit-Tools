...
{
    /// <summary>
    /// Opens "Build Sheets from Zones" on its own STA thread. The zone library is loaded from
    /// the document here, on Revit's main thread, before the window exists.
    /// </summary>
    [Transaction(TransactionMode.Manual)]
    [Regeneration(RegenerationOption.Manual)]
    public class ZoneSheetsCommand : IExternalCommand
    {
        ...
        public static void Open(UIApplication? uiApp)
        {
            ...
            if (_window != null)
            {
                ...
                {
                    DiagnosticsLog.Swallowed("ZoneSheetsCommand: reactivate existing window", ex);
                    _window = null;
                }
            }

            ZoneSheetsViewModel BuildTool()
                => new ZoneSheetsViewModel(App.ZoneSheetsRunHandler, App.ZoneSheetsRunEvent);
