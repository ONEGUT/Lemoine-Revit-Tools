...
> **Superseded in places during implementation and by later work on `main`.** This
> plan is kept as the record of intent; where it disagrees with `installer/`, the
> code is right. What actually shipped differs on four points:
>
> 1. **Install scope** — per-user by default (`PrivilegesRequired=lowest`, no admin),
>    with a "for all users / for me only" chooser on every run. Not admin-only.
> 2. **No staging folder** — the installer packages straight from the csproj's own
>    `DeployDir` (`%ProgramData%\Autodesk\Revit\Addins\<year>\`), naming only
>    Lemoine's own files so the shared folder's other vendors are untouched. There is
>    no `installer\stage\`, and the year guard is `FileExists(...LemoineTools.dll)`,
>    not `DirExists("stage\<year>")`.
> 3. **No WebView2, no `Web\`** — the WebView2 UI layer was removed wholesale for the
>    WPF-only distribution (see `plan-wpf-only-distribution.md`) after this plan was
>    written. Those files no longer exist and are not packaged.
> 4. **`Seed\` is packaged** — added to the deploy shape after this plan was written;
>    `SeedLibrary` reads it from beside the DLL, so it must be installed.
